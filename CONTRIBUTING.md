...
- Login is the agent-side master valve: only `gamer_account` works without a session token. Other tools return `not_logged_in` before `missing_room_id` / `no_ticket`. Platform `GET /v1/rooms` stays public for humans.
...
- Inject platform start/end copy the DSH way: `createUserMessage` + `<system-reminder>`. Wake on `match_started` / `match_ended` only — a ticket is a credential, not a notice. Idle: at most one `followup`, later notices `inject`. Running: `inject` into the current turn. Stall `followup` only if idle 15s while `yourTurn` is true; waiting on an opponent is allowed. Do not proxy `POST /v1/ready` on the game.
